...
export function Watched() {
  ...
  const { user, watchedRows, watchedKeys, watchlistKeys, toggleWatched, toggleWatchlist } = useOutletContext<LayoutContext>()
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
      {watchedRows.map((r) => {
        const item: DiscoverCardItem = {
          ...
          vote_average: 0,
          // Show the date the user watched it — we don't have the original
          // release date on watched_rows.
          date: r.watched_at.slice(0, 10),
        }
